//+------------------------------------------------------------------+
//|                                     GapTrader Pro EA v17.0      |
//|              Mean Reversion Strategy (Dollar-Based Calculations)|
//|              SELL UP Reversion | BUY DOWN Reversion             |
//|              SEPARATE MAX BUY & MAX SELL POSITIONS              |
//|              REBUILD ON TP/SL CLOSE (OPTIONAL)                  |
//|              SPREAD FILTER PROTECTION                           |
//+------------------------------------------------------------------+
#property copyright "GapTrader Pro v17.0 - © Torama Capital"
...
// Input Parameters - Mean Reversion Strategy
input group "=== Mean Reversion Strategy ==="
input ENUM_TRADE_DIRECTION TradeDirection = TRADE_BOTH;
input double GapSizeDollars = 200.0;           // Gap size in dollars for reversion triggers
...
// Mean Reversion Trade Info Structure
struct TradeInfo {
   // Price levels for mean reversion strategy
   double centerPrice;
...
int OnInit() {
   ...
   if(stateRestored) {
      RestoreState();
      Print("*** STATE RESTORED - MEAN REVERSION EA CONTINUING ***");
   } else {
      InitFreshState();
      Print("*** FRESH START - MEAN REVERSION STRATEGY INITIALIZED ***");
   }
...
void InitFreshState() {
   ...
   Print("*** Mean Reversion Strategy Initialized | Center: ", DoubleToString(currentPrice, _Digits), " ***");
}
...
void PrintStrategyInfo() {
   Print("===================================================================");
   Print("GapTrader Pro EA v17.0 - Mean Reversion Strategy - © Torama Capital");
   Print("Email: ea@torama.biz | Web: money.torama.biz");
   Print("===================================================================");
   Print("MEAN REVERSION STRATEGY:");
   Print("- SELL: Opens when price RISES by gap (betting on reversion down)");
   Print("- BUY: Opens when price FALLS by gap (betting on reversion up)");
   Print("-------------------------------------------------------------------");
   Print("Trading Direction: ", TradeDirection == TRADE_BOTH ? "BOTH (Sell Up + Buy Down)" : 
         (TradeDirection == TRADE_BUY ? "BUY REVERSION ONLY" : "SELL REVERSION ONLY"));
   Print("Gap Size: $", DoubleToString(GapSizeDollars, 2), " (reversion trigger)");
...
//+------------------------------------------------------------------+
//| Main Trading Logic - Mean Reversion Strategy                     |
//+------------------------------------------------------------------+
void OnTick() {
   CalcTotalPL();
   ...
   CheckReversionTrades();
}

void CheckReversionTrades() {
   ...
   // *** BUY REVERSION: Open BUY when price moves DOWN by gap amount (betting on bounce up) ***
   if((TradeDirection == TRADE_BUY || TradeDirection == TRADE_BOTH) && 
      ti.buyPositions < MaxBuyPositions) {  // SACROSANCT CHECK
      
      if(currentPrice <= ti.lastBuyPrice - gapInPrice) {  // OPPOSITE: Price FELL
         if(OpenBuy()) {
            ti.lastBuyPrice = currentPrice;
            ti.buyPositions++;
            Print(">>> BUY REVERSION at ", DoubleToString(currentPrice, _Digits), 
   ...
   // *** SELL REVERSION: Open SELL when price moves UP by gap amount (betting on pullback down) ***
   if((TradeDirection == TRADE_SELL || TradeDirection == TRADE_BOTH) && 
      ti.sellPositions < MaxSellPositions) {  // SACROSANCT CHECK
      
      if(currentPrice >= ti.lastSellPrice + gapInPrice) {  // OPPOSITE: Price ROSE
         if(OpenSell()) {
            ti.lastSellPrice = currentPrice;
            ti.sellPositions++;
            Print(">>> SELL REVERSION at ", DoubleToString(currentPrice, _Digits), 
                  " | Gap: $", DoubleToString(GapSizeDollars, 2), 
                  " | Spread: ", GetCurrentSpread(), " pts",
...
bool OpenBuy() {
   ...
   string comment = "GT17-BUY_REVERSION";
...
bool OpenSell() {
   ...
   string comment = "GT17-SELL_REVERSION";
...
void RebuildGrid(string reason = "Manual") {
   ...
   Print("*** MEAN REVERSION GRID REBUILD (", reason, ") | ", DoubleToString(oldPrice, _Digits), 
...
void OnChartEvent(const int id, const long &lparam, const double &dparam, const string &sparam) {
   ...
   if(sparam == "GT_CloseProfitsBtn") {
      ...
      if(ti.isPaused) {
         ti.nextAutoRebuild = 0;
         Print("*** MEAN REVERSION EA PAUSED ***");
      } else {
         RebuildGrid("Resume");
         if(EnableAutoRebuild && ti.autoRebuilds < MaxRebuilds) {
            ti.nextAutoRebuild = TimeCurrent() + (RebuildIntervalMinutes * 60);
         }
         Print("*** MEAN REVERSION EA RESUMED ***");
      }
   }
...
bool CreatePanel() {
   ...
   CreateLabel("GT_Title", "GapTrader Pro v17.0 - Mean Reversion", 10, 8, 11, clrNavy);
   ...
   string labels[] = {
      ...
      "GT_ReversionLevels", "GT_NetAllPositions", "GT_CurrentPrice", "GT_GapSize", 
...
void UpdatePanel() {
   ...
   string statusText = ti.isPaused ? "Status: PAUSED" : "Status: ACTIVE (Mean Reversion)";
   ...
   string dir[] = {"BUY REVERSION ONLY", "SELL REVERSION ONLY", "BOTH (Sell Up + Buy Down)"};
   ...
   string reversionLevels = "Last: Buy@" + DoubleToString(ti.lastBuyPrice, _Digits) + 
                          " Sell@" + DoubleToString(ti.lastSellPrice, _Digits);
   ObjectSetString(0, "GT_ReversionLevels", OBJPROP_TEXT, reversionLevels);
   ...
   // OPPOSITE TRIGGERS: Buy when price falls, Sell when price rises
